...
- [x] Refactor ui.go into smaller helpers — Split the large window construction into helpers for menus, dialogs, history, and preferences so the file is easier to scan and change. *(`showAbout`, `showHistory`, `showConfigHelp`, `showPreferences`, `showPostProcessing`, `createMainMenu`, and now `createUI` have all moved to UIManager; `ui.go` is left with only thin `DownloaderApp` delegates plus the shared `roundedCard` helper. Further internal splitting of `createUI` itself into smaller helpers is tracked separately in roadmap.md.)*
...
**No open next steps** — the service itself is fully self-contained and requires no further extraction. Its *consumers* still have pending work: see PPEngine step 4 (the `runJob` GPU-lifecycle extraction) above; `showPostProcessing` never needed a direct `gpuSvc` reference since it only reads `ui.gpuBackend`/`prefs.GPUBackend`.

> **Coupling note (resolved):** UIManager step 5 confirmed `UIManager` never needed a direct `gpuSvc *GPUCapabilityService` reference — `showPostProcessing` only touches `ui.gpuBackend` and `prefs.GPUBackend`, both already decoupled via the `onLoadPreferences` callback.
